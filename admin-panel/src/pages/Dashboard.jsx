...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const [blogsRes, productsRes, reviewsRes, categoriesRes] = await Promise.all([
          fetch(`${baseURL}/blogs`),
        ...
        const blogsCount = Array.isArray(blogs) ? blogs.length : blogs?.data?.length || 0;
        const productsCount = Array.isArray(products) ? products.length : products?.data?.length || 0;
        const reviewsCount = Array.isArray(reviews) ? reviews.length : reviews?.data?.length || 0;
        const categoriesCount = Array.isArray(categories) ? categories.length : categories?.data?.length || 0;
  ...
  const statCards = [
    { label: "Blogs", count: stats.blogs, color: "from-[#2C498D] to-[#3e5fab]" },
    { label: "Products", count: stats.products, color: "from-green-500 to-emerald-400" },
    { label: "Reviews", count: stats.reviews, color: "from-yellow-500 to-amber-400" },
    { label: "Categories", count: stats.categories, color: "from-purple-500 to-indigo-500" },
  ];
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-slate-50 to-slate-200 flex justify-center items-center p-4">
      <div className="w-full max-w-6xl h-full bg-white rounded-3xl shadow-2xl flex flex-col p-6 md:p-10 border border-gray-200">
        ...
        >
          Real-time insights for <strong>Weighing Scales</strong>, <strong>CCTV Solutions</strong> & <strong>Poultry Equipment</strong>.
        </motion.p>
        ...
        >
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.plugins} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
        ...
        <div className="text-xs text-gray-500 border-t pt-4 flex justify-center items-center gap-4">
          <span className="text-[#2C498D] font-semibold">Admin Access for JK Enterprises</span>
          <span>Developed by <strong>Dashon Solutions</strong></span>
          <span>
            Support:{" "}
            <a href="mailto:clientdesk.team@gmail.com" className="text-[#2C498D] underline">
