...
const WhyChooseUsList = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteWhyChooseUs(deletingItemId);
      ...
      toast.update(toastId, {
        render: error?.response?.data?.message || "Failed to delete item.",
        type: "error",
        isLoading: false,
        autoClose: 5000,
      });
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
        <div className="bg-white p-6 rounded-xl shadow-2xl text-center space-y-4 max-w-md w-full mx-4">
          ...
          <p className="text-slate-600">Are you sure you want to delete this <span className="text-sm font-bold text-slate-800">{title}</span> item?</p>
    ...
    <div className="min-h-screen  p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
        <div>
          ...
          <p className="text-slate-600 mt-2 text-lg">Manage your competitive advantages</p>
        </div>
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden">
                {item?.iconUrl ? (
                  <img
                    src={item.iconUrl}
                    alt={item.title?.[selectedLang] || item.title?.en || 'Item icon'}
                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                    onError={(e) => {
                      e.target.src = 'https://via.placeholder.com/400x200?text=No+Icon';
                    }}
                  />
                ) : (
              ...
              <div className="p-6 space-y-4">
                ...
                <h3 className="text-xl font-bold text-slate-800 line-clamp-2 group-hover:text-blue-700 transition-colors duration-300">
                  {item.title?.[selectedLang] || item.title?.en || 'Untitled Item'}
                </h3>

                {/* Description */}
                {(item.description?.[selectedLang] || item.description?.en) && (
                ...
                {selectedLang !== "en" && item.title?.en && (
                  <p className="text-slate-500 italic line-clamp-1 text-xs">
      ...
      {!loading && items.length === 0 && (
        <div className="text-center py-20">
          ...
          <h3 className="text-2xl font-bold text-slate-800 mb-3">No items yet</h3>
          <p className="text-slate-600 mb-8 text-lg max-w-md mx-auto">
            Create your first "Why Choose Us" item to showcase your competitive advantages.
          </p>
          <button
            onClick={() => setShowAdd(true)}
      ...
      {/* Modals */}
      <Modal title="Add Why Choose Us" isOpen={showAdd} onClose={() => setShowAdd(false)}>
        <WhyChooseUsForm
          onSuccess={() => {
            setShowAdd(false);
            fetchData();
      ...
      <Modal title="Edit Why Choose Us" isOpen={!!editData} onClose={() => setEditData(null)}>
        <WhyChooseUsEdit
          data={editData}
          onSuccess={() => {
            setEditData(null);
