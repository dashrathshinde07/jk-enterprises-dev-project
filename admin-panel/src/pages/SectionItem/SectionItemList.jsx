...
const SectionItemList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast.error("Failed to fetch section items");
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    const toastId = toast.loading("Deleting section item...");
    try {
      await deleteSectionItem(deletingReviewId);

      toast.update(toastId, {
        render: "Section item deleted successfully!",
        type: "success",
        isLoading: false,
        autoClose: 3000,
      });
      ...
      toast.update(toastId, {
        render: error?.response?.data?.message || "Failed to delete section item.",
        type: "error",
        isLoading: false,
        autoClose: 5000,
      });
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
        <div className="bg-white p-6 rounded-xl shadow-2xl text-center space-y-4 max-w-md w-full mx-4">
          ...
          <h3 className="text-xl font-bold text-slate-800">Delete Section Item</h3>
          <p className="text-slate-600">
            Are you sure you want to delete this <span className="text-sm font-bold text-slate-800">{SectionTitle}</span> section item? This action cannot be undone.
          </p>
    ...
    <div className="min-h-screen p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
        <div>
          ...
          <p className="text-slate-600 mt-2 text-lg">Manage your section content items</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-white/20 overflow-hidden">
            <button
              ...
              className={`px-5 py-3 text-sm font-semibold transition-all duration-300 ${selectedLang === "en"
            ...
            <button
              ...
              className={`px-5 py-3 text-sm font-semibold transition-all duration-300 ${selectedLang === "mr"
                ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg"
                : "text-slate-600 hover:bg-white/60"
                }`}
            >
          ...
          <button
            ...
            className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-6 py-3 rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 flex items-center gap-2 transform hover:scale-[1.02]"
          >
            <Plus size={20} />
            Add Item
          </button>
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden">
                <img
                  src={item.imageUrl}
                  alt={item?.title?.[selectedLang] || item?.title?.en || "Section item"}
                ...
                <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                  ...
                  <button
                    onClick={() => handleDelete(item._id, item?.title?.en)}
                    className="bg-white/90 backdrop-blur-sm p-2 rounded-lg hover:bg-white shadow-lg transition-all duration-200 hover:scale-110"
                    title="Delete item"
                  >
              ...
              <div className="p-6 space-y-4">
                ...
                <h3 className="text-xl font-bold text-slate-800 line-clamp-2 group-hover:text-blue-700 transition-colors duration-300">
                  {item?.title?.[selectedLang] || item?.title?.en || item?.title?.mr || 'Untitled Item'}
                </h3>
                ...
                <div className="space-y-2">
                  {selectedLang === "en" ? (
                    <>
                      {item?.title?.en && (
                        <div>
                          <span className="text-xs font-medium text-blue-600 uppercase tracking-wide bg-blue-50 px-2 py-1 rounded-md">
                            English
                          </span>
                      ...
                      {item?.title?.mr && (
                        <div>
                          <span className="text-xs font-medium text-indigo-600 uppercase tracking-wide bg-indigo-50 px-2 py-1 rounded-md">
                            मराठी
                          </span>
      ...
      {!loading && items.length === 0 && (
        <div className="text-center py-20">
          ...
          <h3 className="text-2xl font-bold text-slate-800 mb-3">No section items yet</h3>
          <p className="text-slate-600 mb-8 text-lg max-w-md mx-auto">
            Create your first section item to organize and display your content effectively.
          </p>
          <button
            onClick={() => setShowAdd(true)}
